        ...
        private void DrawSummaryStats(Graphics g, int yPos)
        {
            ...
            using (Pen borderPen = new Pen(Color.FromArgb(0, 120, 212), 2))
            {
                ...
                if (allTemps.Any())
                {
                    ...
                    DrawStatItem(g, "Average Temperature:", $"{avgTemp:F1}°C", statX, statY, GetTemperatureColor(avgTemp));
                    DrawStatItem(g, "Maximum Temperature:", $"{maxTemp:F1}°C", statX + columnWidth, statY, GetTemperatureColor(maxTemp));
                    DrawStatItem(g, "Minimum Temperature:", $"{minTemp:F1}°C", statX + columnWidth * 2, statY, GetTemperatureColor(minTemp));
            ...
            using (SolidBrush detailBrush = new SolidBrush(Color.FromArgb(108, 117, 125)))
            {
                ...
                if (history.Count > 0)
                {
                    ...
                    if (temps.Any())
                    {
                        ...
                        string details = $"Min: {min:F1}°C | Max: {max:F1}°C";
